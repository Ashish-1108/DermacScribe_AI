...
st.warning("For exploring the website use sidebar")
...
image_path = os.path.join(script_dir, "home1.jpg")
...
# Load the second image for the Analyzer section
analyzer_image_path = os.path.join(script_dir, "home2.webp")
analyzer_image = load_local_image(analyzer_image_path)

# Load the third image for the FaceScan section
facescan_image_path = os.path.join(script_dir, "home3.png")
facescan_image = load_local_image(facescan_image_path)
...
def show_welcome_page():
    ...
    with analyzer_cols[0]:
        if analyzer_image: 
            st.image(analyzer_image, use_container_width=True, caption="Analyze Your Skincare Routine") 
        else:
    ...
    with facescan_cols[1]:
        if facescan_image: # <-- CHANGED to use facescan_image
            st.image(facescan_image, use_container_width=True, caption="Discover Personalized Recommendations") # <-- CHANGED
        else:
            st.markdown('<div style="height: 400px; display: flex; align-items: center; justify-content: center; background: var(--color-border); border-radius: 15px;"><p style="color: var(--color-text-medium);">Image placeholder</p></div>', unsafe_allow_html=True)
